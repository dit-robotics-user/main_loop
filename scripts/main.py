...
# ==設定任務及動作==
action_list, current_world_state, mission_list, child_list, my_pos = setting(setting_number, action_list, child_list, current_world_state, mission_list, my_pos, my_degree, cup_color, direction)
...
while 1:
    ...
    while path_done is False:
        ...
        if time >= go_home_time and go_home_flag is False:
            path_done = True  # 跳出WHILE並重算
            go_home_flag = True
            continue
        ...
        if len(path) != 0:
            ...
            while len(top_path.child_action) != 0:
                ...
                if action_done is True:
                    ...
                    if output_name == 'lift_left':
                        current_left_layer -= 1
                    elif output_name == 'lift_right':
                        current_right_layer -= 1
                    elif output_name == 'lower_left':
                        current_left_layer += 1
                    elif output_name == 'lower_right':
                        current_right_layer += 1
