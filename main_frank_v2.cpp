   ...
   // goap_srv.request.cup_color = {}; 
	...
	while(ros::ok()){
				...
				if(sub.client_goap.call(goap_srv)){
		            ...
		            waiting = bool(goap_srv.response.is_wait);
		        }
		        else{
		...
		*/
		//-------------------------------------------------------------------------path
		      ...
		      /*  sub.n.getParam("main_frank/goal_pos_x", path_srv.request.goal_pos_x);
				...
				ROS_INFO("waiting: %d", waiting);
				if(waiting == true && at_pos(my_pos_x, my_pos_y, my_degree, path_srv.request.goal_pos_x, path_srv.request.goal_pos_y, goap_srv.response.degree, margin, angle_margin)){
