...
class sub_class{
    public:
        ...
        //become add
        void ally_ST1_sub_callback(const std_msgs::Int32MultiArray::ConstPtr& msg);//subsribe "/small/rxST1"(std_msgs::Int32MultiArray)
        ...
        void strategy_sub_callback(const std_msgs::Int32::ConstPtr& msg);//subscribe strategy
    ...
    private:
        ...
        //become add
        ros::Subscriber ally_ST1_sub = n.subscribe<std_msgs::Int32MultiArray>("/small/rxST1", 1, &sub_class::ally_ST1_sub_callback,this);
...
void sub_class::change_cup_color(int color[]){ // change this->pub_to_main.cup_color
    ...
    for(int k=0;k<5;k++){
        this->pub_to_main.cup_color.push_back(color[k]);
    }
}
...
void sub_class::ally_ST1_sub_callback(const std_msgs::Int32MultiArray::ConstPtr& msg){ //subsribe "/small/rxST1"(std_msgs::Int32MultiArray)
...
void sub_class::publish_(float time ){  //publish data to main
    this->pub_to_main.time =time ; 
    this->status.data = now_status();
    pub_to_main.status = now_status() ;
    if(status.data!=5){
        pub_to_main.emergency={};
...
int main(int argc, char **argv){
    ...
    int color_[5]={1,0,0,0,0};//
    temp.change_cup_color(color_);
	
    //availible form 
    //N S : 0 1 
    //color green red : 0 1 
    //ex: cup color : 0 0 1 0 0  :G G R G G
    //transfer form
    ...
    while(ros::ok()){
        ...
        temp.status_publish();   // publish status to GUI node
        ros::spinOnce();
    }
}
