...
def output_processor(output_action, left_side, right_side):
    ...
    if output_action.name is 'lift_left':
        ...
        if right_side is 0:
            output[12] = 0
        if right_side is 1:
            output[12] = 0  # lift layer 3
        elif right_side is 2:
            output[12] = 1  # lift layer 2
    elif output_action.name is 'lower_right':
        if right_side is 0:
            output[13] = 0
        if right_side is 1:
    ...
    while path_done is False:
        if action_done is True:
            ...
            if path.name is 'lift_left':
                ...
                right_side += 1
            path_done = True
            if len(demo_path) > 1 and last_action_done is not True:
                demo_path.remove(demo_path[0])
    last_action_done = action_done
